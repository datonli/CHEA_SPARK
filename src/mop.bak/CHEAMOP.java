...
public class CHEAMOP extends MOP{
    ...
    public int tourSelectionHV(List<SOP> sops) {
		...
		//System.out.println("next step tourSelectionHVDifference");
        double hv1 = tourSelectionHVDifference(p1,sops);
        double hv2 = tourSelectionHVDifference(p2,sops);
		//System.out.println("end tourSelectionHVDifference");
    ...
    public double tourSelectionHVDifference(int p,List<SOP> sops){
			...
			// Nov 28 remove this part for mement
            //while(sops.get(p).ind.belongSubproblemIndex != sops.get(p).sectorialIndex) {
			//	System.out.println("tourSelectionHVDifference drop ???, p is : " + p);
...
*/
	...
	public void evolutionTourSelect2() {
        ...
        for(int i = 0 ;i < popSize; i ++){
            ...
            int parentIndex2 ;
            int b = len % (popSize/7); 
			//System.out.println("next step: tourSelectionHV");
            if(b < sizeSubpOnEdge) {
			...
			//System.out.println("end tourSelectionHV");
			...
			//System.out.println("after evaluate,before partition");
			...
			//if(0 == offSpring.belongSubproblemIndex) System.out.println("offSpring 's belongSubproblemIndex is zero, in " + len);
	...
	public void updatePartition() {
		...
		for(int n = 0 ; n < popSize; n ++) {
			if(sops.get(n).sectorialIndex == sops.get(n).ind.belongSubproblemIndex ){
				...
				// had fix it Nov 20
				while(true) {
				//	System.out.println("maybe drop in,ind's belongSubproblemIndex is : " + ind.belongSubproblemIndex);
		...
		for(int i = 0 ; i < popSize; i ++) {
			if(false  == sopsFlag[i] ) {
				...
				//System.out.println("partition 's i : " + i + ",minDiffIndex is : " + minDiffIndex);
